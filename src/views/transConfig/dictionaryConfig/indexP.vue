    ...
    <el-card shadow="always">
      ...
      <el-table border :data="data" :loading="loading" :cell-style="cellstyle" :header-cell-style="rowClass" size="small">
        ...
        <el-table-column label="操作" width="250" fixed="right">
          <template slot-scope="scope">
            <a v-if="scope.row.rspType==1" class="linkfont" style="margin-right:30px" @click="openDrawer(scope.row)">修改</a>
            <a v-if="scope.row.rspType==1" class="linkfont" @click="handleRemove(scope.row)">删除</a>
          </template>
        </el-table-column>
      </el-table>
