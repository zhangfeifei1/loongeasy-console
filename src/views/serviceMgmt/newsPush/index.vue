  ...
  <div class="app-container wrapper">
    <el-tabs v-model="activeName" type="border-card" @tab-click="handleTabClick">
      ...
      <el-tab-pane label="消息列表" name="first">
        ...
        <div style="margin:15px 0px 15px 0px">
          <el-alert title="消息列表默认展示极简推送、模板推送的最近30天历史消息记录；支持查询极简推送、模板推送、批量推送的最近30天历史消息记录" type="warning" show-icon />
        </div>
        <!--数据详情-->
          ...
          <el-table :data="data" border :cell-style="cellstyle" :header-cell-style="rowClass" size="small">
            ...
            <el-table-column prop="targetType" label="维度" width="150">
              <template slot-scope="scope">
                <span v-if="scope.row.targetType==='1'">用户</span>
                <span v-if="scope.row.targetType==='2'">设备</span>
              </template>
            </el-table-column>
            <el-table-column prop="targetId" label="推送目标 ID" width="200">
              <template slot="header">
                <el-tooltip class="item" effect="dark" content="推送目标 ID 分为用户标识及设备标识两种" placement="top">
                  <span>推送目标 ID</span>
                </el-tooltip>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="status" label="消息推送状态" width="200">
              <template slot-scope="scope">
                <span v-if="scope.row.status==='DeviceNotOnlineOrNoResponse'">DeviceNotOnlineOrNoResponse</span>
                <span v-if="scope.row.status==='NoBindInfo'">NoBindInfo</span>
                <span v-if="scope.row.status==='Acked'">Acked</span>
                <span v-if="scope.row.status==='ConnClosed'">ConnClosed</span>
                <span v-if="scope.row.status==='BadDeviceToken'">BadDeviceToken</span>
                <span v-if="scope.row.status==='DeviceTokenNotForTopic'">DeviceTokenNotForTopic</span>
              </template>
            </el-table-column>
            <el-table-column prop="expiTime" label="消息过期时间" width="200" />
            <el-table-column prop="updateTime" label="消息变更时间" width="200">
              <template slot="header">
                <el-tooltip class="item" effect="dark" content="消息变更时间分为三种情况，分别是即时成功、即时失败、超时失败" placement="top">
                  <span>消息变更时间</span>
                </el-tooltip>
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="使用分析" name="second">
        <el-row style="margin:14px 0px 10px 0px">
          ...
          <el-col :span="3">
            <template>
              <el-select v-model="secondSearch.pushChannel" size="small" placeholder="所有推送渠道" clearable>
          ...
          <el-col :span="3" style="margin:0px 20px 0px 20px">
            <template>
              <el-select v-model="secondSearch.pushType" size="small" placeholder="所有推送类型" clearable>
        ...
        <div class="tableStyle">
          ...
          <el-row style="height:110px;" type="flex" justify="center">
            <el-col :span="5" style="background-color:#F0F6FE;margin-right:10px">
              <div style="float:left;margin-right:15px;margin-left:20px">
                <i class="el-icon-data-line" style="line-height:110px;font-size:80px;font-weight:300;color:#5f9bf1" />
              </div>
          ...
          <el-table :data="secondData" border :cell-style="cellstyle" :header-cell-style="rowClass" size="small">
      ...
      <el-tab-pane label="消息模板" name="third">
        <el-row>
          <el-col :span="4">
            <el-button size="small" type="primary" icon="el-icon-plus" style="margin-bottom:10px" @click="thirdAdd()">创建模板</el-button>
          </el-col>
        </el-row>
        <!--数据详情-->
        <div class="tableStyle">
          <el-table :data="thirddata" border :cell-style="cellstyle" :header-cell-style="rowClass" size="small">
            <el-table-column prop="tempName" label="模板名称" width="220" />
            <el-table-column prop="tempDesc" label="模板描述" width="250" />
      ...
      <el-tab-pane label="推送配置" name="fourth">
        <el-card style="margin-bottom:20px">
          <div slot="header">
            <span>FCM 推送渠道</span>
            <el-button style="float: right; padding: 3px 0" type="text" @click="fourthConfigure1()">配置</el-button>
          </div>
          <div>
            <el-row v-if="!fourthConfigureFlag1" type="flex" justify="center">
              <h5>mPaaS 移动推送服务接入了 FCM 推送渠道，可以在安装有 Google Play 服务的手机上建立系统级长连接，极大提高了海外手机设备的消息送达率。
              </h5>
            </el-row>
            ...
            <el-row v-if="fourthConfigureFlag1" type="flex" justify="center">
              <div>
                <template>
                  ...
                  <el-button type="primary" style="margin-left:230px" size="small" @click="handleFourthConfig()">确 定
                  </el-button>
                </template>
              </div>
            </el-row>
              ...
              <div>
                <template>
                  ...
                  >
                    ...
                    <el-form-item label="证书密码：" prop="IOSPassword">
                      <el-input v-model="IOSFrom.IOSPassword" style="width:400px" />
                    </el-form-item>
                    <el-form-item label="证书环境：" prop="IOSEnvir">
                      <el-select v-model="IOSFrom.IOSEnvir" style="width:260px" @change="IOSChange()">
                  ...
                  <el-button type="primary" style="margin-left:146px" size="small" @click="handleIOS()">确 定
                  </el-button>
                </template>
              </div>
            </el-row>
      ...
      <el-tab-pane label="渠道配置" name="sixth">
        <el-card style="margin-bottom:20px">
          <div slot="header">
            <span>小米推送渠道</span>
            <el-button style="float: right; padding: 3px 0" type="text" @click="sixthConfigure1()">配置</el-button>
          </div>
          <div>
            <el-row v-if="!sixthConfigureFlag1" type="flex" justify="center">
              <h5>mPaaS 移动推送服务接入了小米推送渠道，可以在小米手机上建立系统级长连接，极大提高小米手机的消息送达率。
              </h5>
            </el-row>
            ...
            <el-row v-if="sixthConfigureFlag1" type="flex" justify="center">
              <div>
                <template>
                  <el-form ref="sixthConfig1" :model="sixthConfig1" label-width="200px" label-position="right" size="small">
                    <el-form-item required label="状态：" prop="status">
                  ...
                  <el-button size="small" type="primary" style="margin-left:230px" @click="handleSixthConfig1()">确 定
                  </el-button>
                </template>
              </div>
            </el-row>
        ...
        <el-card style="margin-bottom:20px">
          <div slot="header">
            <span>华为推送渠道</span>
            <el-button style="float: right; padding: 3px 0" type="text" @click="sixthConfigure2()">配置</el-button>
          </div>
          <div>
            <el-row v-if="!sixthConfigureFlag2" type="flex" justify="center">
              <h5>mPaaS 移动推送服务接入了华为推送渠道，可以在华为手机上建立系统级长连接，极大提高华为手机的消息送达率。
              </h5>
            </el-row>
            ...
            <el-row v-if="sixthConfigureFlag2" type="flex" justify="center">
              <div>
                <template>
                  <el-form ref="sixthConfig2" :model="sixthConfig2" label-width="200px" label-position="right" size="small">
                    <el-form-item required label="状态：" prop="status">
                  ...
                  <el-button size="small" type="primary" style="margin-left:230px" @click="handleSixthConfig2()">确 定
                  </el-button>
                </template>
              </div>
            </el-row>
        ...
        <el-card style="margin-bottom:20px">
          <div slot="header">
            <span>OPPO推送渠道</span>
            <el-button style="float: right; padding: 3px 0" type="text" @click="sixthConfigure3()">配置</el-button>
          </div>
          <div>
            <el-row v-if="!sixthConfigureFlag3" type="flex" justify="center">
              <h5>mPaaS 移动推送服务接入了OPPO推送渠道，可以在OPPO手机上建立系统级长连接，极大提高OPPO手机的消息送达率。
              </h5>
            </el-row>
            ...
            <el-row v-if="sixthConfigureFlag3" type="flex" justify="center">
              <div>
                <template>
                  <el-form ref="sixthConfig3" :model="sixthConfig3" label-width="200px" label-position="right" size="small">
                    <el-form-item required label="状态：" prop="status">
                  ...
                  <el-button size="small" type="primary" style="margin-left:230px" @click="handleSixthConfig3()">确 定
                  </el-button>
                </template>
              </div>
            </el-row>
        ...
        <el-card style="margin-bottom:20px">
          <div slot="header">
            <span>VIVO推送渠道</span>
            <el-button style="float: right; padding: 3px 0" type="text" @click="sixthConfigure4()">配置</el-button>
          </div>
          <div>
            <el-row v-if="!sixthConfigureFlag4" type="flex" justify="center">
              <h5>mPaaS 移动推送服务接入了VIVO推送渠道，可以在VIVO手机上建立系统级长连接，极大提高VIVO手机的消息送达率。
              </h5>
            </el-row>
            ...
            <el-row v-if="sixthConfigureFlag4" type="flex" justify="center">
              <div>
                <template>
                  <el-form ref="sixthConfig4" :model="sixthConfig4" label-width="200px" label-position="right" size="small">
                    <el-form-item required label="状态：" prop="status">
                  ...
                  <el-button size="small" type="primary" style="margin-left:230px" @click="handleSixthConfig4()">确 定
                  </el-button>
                </template>
              </div>
            </el-row>
              ...
              <el-collapse v-model="colName" accordion>
                  ...
                  <div>
                    <template>
                      <el-form
                        ...
                        :rules="colValidate"
                      >
                        <el-form-item v-if="newPushActive!='newPushFourth'" label="目标ID类型:" prop="targetType">
                          <el-select
                          ...
                          >
                            ...
                            <el-option label="DeviceId" value="DeviceId" />
                          </el-select>
                        </el-form-item>
                        <el-form-item
                          v-if="colModel.targetType=='DeviceId'||newPushActive=='newPushFourth'"
                          label="推送平台:"
                          prop="pushFramework"
                        >
                          <el-select
                        ...
                        <el-form-item v-if="newPushActive!='newPushThird'" label="业务方消息ID:" prop="bussMsgId">
                          <el-input v-model="colModel.bussMsgId" size="mini" style="width:260px">
                            ...
                            <template v-if="newPushActive=='newPushFourth'" slot="prepend">broadcast_</template>
                          </el-input>
                        </el-form-item>
                        <el-form-item
                        ...
                        <el-form-item v-if="newPushActive!='newPushFirst'" label="推送模板:" prop="template">
                          <el-select v-model="colModel.template" size="mini" style="width:260px" @change="mubanChange()">
                            <el-option
                        ...
                        <el-form-item v-if="newPushActive=='newPushThird'" label="选择推送人群:" prop="userGroup">
                          <template>
                            <el-radio-group v-model="colModel.userGroup" style="width:280px" @change="pushManChange()">
                        ...
                        >
                          <template>
                            <el-select v-model="colModel.templateId" size="mini" style="width:260px">
                              <el-option
                        ...
                        <el-form-item v-if="colModel.pushFramework!='IOS'" label="点击后操作:" prop="action">
                          <template>
                            <el-radio-group
                        ...
                        <el-form-item v-if="newPushActive=='newPushFirst'" label="推送标题:" prop="title">
                          <el-input v-model="colModel.title" size="mini" style="width:260px" />
                        </el-form-item>
                        <el-form-item v-if="newPushActive=='newPushFirst'" label="推送文本:" prop="content">
                          <el-input v-model="colModel.content" type="textarea" :rows="2" style="width:260px" />
                        </el-form-item>
                      </el-form>
                    </template>
                  </div>
                ...
                <el-collapse-item name="col2">
                  ...
                  <div style="margin-top:12px">
                    <template>
                      <el-form ref="seniorModel" :model="seniorModel" label-width="120px" label-position="left">
                        <el-form-item v-if="newPushActive=='newPushFirst'" label="跳转地址:" prop="jumpUrl">
                          <el-input v-model="seniorModel.jumpUrl" size="mini" style="width:260px" />
                        </el-form-item>
                        <el-form-item label="消息有效期:" prop="expDate">
                          <el-input-number v-model="seniorModel.expDate" size="mini" /> ( s )
                        </el-form-item>
                        <el-alert style="margin-bottom:20px" title="消息有效期不得超过 72 小时。" type="warning" />
                        <el-form-item v-if="newPushActive=='newPushFourth'" label="推送登录状态:" prop="loginStatus">
                          <template>
                            <el-radio-group v-model="seniorModel.loginStatus" @change="pushTypeChange()">
                        ...
                        <el-form-item
                          ...
                          prop="logoutTime"
                        >
                          <el-select v-model="seniorModel.logoutTime" size="mini" style="width:260px">
                      ...
                      <div v-if="seniorModel.extStatus==true" class="tableStyle">
                        <el-table :data="extParam" border :cell-style="cellstyle" :header-cell-style="rowClass">
            ...
            <el-card shadow="hover">
              <el-tabs v-model="muban" @tab-click="mubanClick">
                <el-tab-pane label="通知" name="muban1">
                  <div style="padding:30px;margin:10px 0px 10px 0px;width:85%;background-color:#cbdaf1">
                    <div style="height:400px;border:3px solid #000000;border-radius:16px;background-color:#A0A0A0">
                ...
                <el-tab-pane label="苹果消息体" name="muban2">
                  <div style="padding:30px;margin:10px 0px 10px 0px;width:85%;background-color:#cbeef1">
                ...
                <el-tab-pane label="安卓消息体" name="muban3">
                  <div style="padding:30px;margin:10px 0px 10px 0px;width:85%;background-color:#cef1cb">
                    <div style="padding:30px 20px 30px 20px;background-color:#f7f7f7">
                      <pre
                        style="white-space:pre-wrap;line-height:22px;font-family:Microsoft YaHei;"
                      >{{ androidMessage }}</pre>
    ...
    <el-dialog :title="thirdDialog" :visible.sync="isAddMuban" width="50%">
      <el-alert
        ...
        show-icon
      />
      <el-form ref="thirdFrom" :model="thirdFrom" label-width="110px" label-position="left" :rules="thirdFromRules">
        <el-form-item label="模板名称:" prop="tempName">
          <el-input
            v-model="thirdFrom.tempName"
        ...
        <el-form-item label="点击后操作:" prop="action">
          <template>
            <el-radio-group v-model="thirdFrom.action" :disabled="thirdDialog=='查看模板'" size="mini">
              <el-radio label="activity">打开Intent Activity</el-radio>
              <el-radio label="webUrl">打卡Web URL</el-radio>
            </el-radio-group>
          </template>
        </el-form-item>
        ...
        <el-form-item label="展示类型:" prop="viewType">
          <template>
            <el-radio-group v-model="thirdFrom.viewType" :disabled="thirdDialog=='查看模板'" size="mini">
              <el-radio label="notiMsg">展示消息（通知栏消息）</el-radio>
              <el-radio label="sileMsg">静默消息</el-radio>
            </el-radio-group>
          </template>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button v-if="thirdDialog=='查看模板'" size="small" type="primary" @click="isAddMuban = false">确 定</el-button>
        <el-button v-if="thirdDialog=='创建模板'" size="small" type="primary" @click="clickAddmuban()">创 建</el-button>
      </span>
    </el-dialog>
  </div>
</template>>
...
export default {
  ...
  computed: {
    showMessagetitle() {
      debugger
      if (this.colModel.template !== undefined && this.colModel.template !== '') {
        let res = ''
    ...
    IOSMessage() {
      ...
      debugger
      if (this.seniorModel.jumpUrl !== '' && this.seniorModel.jumpUrl !== undefined) {
  ...
  methods: {
    ...
    handleSearch() {
      ...
      newsListSearch(params).then(res => {
        debugger
    ...
    thirdSearch() {
      ...
      mubanPageSearch(params).then(res => {
        this.thirddata = res.data.records
        this.thirdpageInfo.total = res.data.total
      }).finally(() => {})
    },
    // 第三表格查看
    thirdTable(val) {
    ...
    handleAdd() {
      ...
      if (this.newPushActive !== 'newPushFourth') {
        this.$set(this.colModel, 'targetType', 'UserId')
      }
      if (this.newPushActive === 'newPushFourth') {
      ...
      if (this.newPushActive === 'newPushFirst' || this.newPushActive === 'newPushSecond') {
    ...
    addNewPushSubmit() {
      this.$refs['colModel'].validate(valid => {
        if (valid) {
          ...
          debugger
          if (this.newPushActive === 'newPushFirst') {
            this.$set(this.colModel, 'msgType', 'S')
          }
          if (this.newPushActive === 'newPushSecond') {
            this.$set(this.colModel, 'msgType', 'T')
          }
          if (this.newPushActive === 'newPushThird') {
            this.$set(this.colModel, 'msgType', 'B')
          }
          if (this.newPushActive === 'newPushFourth') {
            this.$set(this.colModel, 'msgType', 'G')
          }
          debugger
    ...
    // 目标ID类型改变
    tragetIDTypeChange() {
      if (this.colModel.targetType === 'DeviceId' && this.newPushActive === 'newPushThird') {
        this.pushManFlag = true
        this.$set(this.colModel, 'userGroup', 'uploadUser')
      }
      if (this.colModel.targetType === 'UserId' && this.newPushActive === 'newPushThird') {
        this.$set(this.colModel, 'userGroup', '')
        this.pushManFlag = false
      }
      if (this.colModel.targetType === 'DeviceId') {
        this.$set(this.colModel, 'pushFramework', 'Android')
      }
    },
    // 推送平台改变
    ...
    // 推送人群改变
    pushManChange() {
      if (this.colModel.userGroup === 'tagUser' && this.newPushActive === 'newPushThird') {
        this.$set(this.colModel, 'templateId', '')
        // 推送人群
        this.pushMan2Options = []
        // 获取推送人群
        searchGroupMuban().then(res => {
    ...
    pushTypeChange() {
      debugger
      if (this.seniorModel.loginStatus === 'loginout' && this.newPushActive === 'newPushFourth') {
        this.$set(this.seniorModel, 'logoutTime', '')
      }
      if (this.seniorModel.loginStatus === 'login' && this.newPushActive === 'newPushFourth') {
